...
with open(os.path.join(args.output, "panoptic_order_json.txt"), "w") as f:
    f.write("")
with open(os.path.join(args.output, "panoptic.txt"), "w") as f:
    f.write("")
for image in tqdm(list(segment_info.keys())):
    segment = segment_info[image]
    image_path = image.replace("-", "/")

    depth_path = os.path.join(args.depth, image_path.replace(".png", ".jpg"))
    ...
    # 计算每个颜色与输入像素之间的欧氏距离
    if depth.shape[0] * depth.shape[1] > 1000000:
    ...
    assert np.max(panoptic_mask) == len(segment), f"{image_path} panoptic: {np.max(panoptic_mask)}, segment: {len(segment)}"
    ...
    for seg in segment:
        ...
        category = categories[seg["category_id"]]
    ...
    with open(os.path.join(args.output, "panoptic_order_json.txt"), "a") as f:
        f.write("<IMG>" + image_path + "<IMG>" + depth_answer + "\n")
    with open(os.path.join(args.output, "panoptic.txt"), "a") as f:
        f.write("<IMG>" + image_path + "<IMG>" + seg_answer + "\n")

json.dump(segment_json, open(os.path.join(args.output, "panoptic_depth.json"), "w"))